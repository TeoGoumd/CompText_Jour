...
writeLines(text, "moley_news.txt")
...
files <- list.files("./extracted", pattern="*.txt") %>% 
  as.data.frame() |> 
  rename(filename = 1) |> 
...
write.csv(articles_df, "articles.csv")
...
bigrams_df <- articles_df %>%
  ...
  mutate(sentence = str_replace_all(sentence, c(
    "copyright" = "",
    "https" = "",
    "language" = "",
    "english" = "",
...
ggplot(bigrams_df, aes(x = reorder(bigram, n), y = n)) +
  ...
  labs(
    ...
    x = "Bigram",
    y = "Frequency",
    caption = "Source: News Articles Analysis. n=32. By Teona Goderdzishvili"
  ) +
  # Customize theme
  theme_minimal() +
  theme(
