...
most_calls_day <- SF %>%
  count(call_time, name = "call_count") %>%
...
most_calls <- SF %>%
count(call_date, name = "call_count") %>%
  arrange(desc(call_count)) %>%
  ggplot(aes(x=Year, y=n, fill=n))+
  geom_col(position = "dodge")+
labs(title = "The Day with the Most 311 Calls")
...
top_5_days <- SF %>%
  count(call_time, name = "call_count") %>%
  slice_max(order_by = call_count, n = 5) %>%
  arrange(desc(call_count))
...
fewest_calls <- SF %>%
  count(call_time, name = "call_count") %>%
  slice_min(order_by = call_count, n = 5) %>%
  arrange(desc(call_count))
