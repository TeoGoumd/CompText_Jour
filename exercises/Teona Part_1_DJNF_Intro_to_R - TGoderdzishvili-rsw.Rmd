...
#RSW comment - need to load the rio and janitor libraries.
library(rio)
library(janitor)
...
#RSW comment - need to have this command
#colnames(MediaBucks)
```{r}
...
#RSW Comment: This version will display a table just with Gender and Salary
# MediaBucks %>% 
#   select(Gender, Salary) %>% 
#   group_by(Gender) %>% 
#   summarize(Avg_Salary = mean(Salary, na.rm=TRUE))
...
#RSW comment - make sure to leave a space between code chunks to you don't get confused about the order of the code
...
#RSW Comment: This code builds a table and creates it as an object
...
#RSW comment
#It would be easier to get the median from a table that's already created
summary(NYT$Salary)
...
#This is a more efficient way based on the salary metrics you determined
#summary(NYT$Salary)
...
#RSW comment. Please write out the answer:
...
#RSW comment - This is how I did it
# Reporters %>% 
...
#RSW comment: This is how I did it, made sure the search was not case sensitive
...
#Senior Photo Editor	Tribune Publishing	770000	
...
#RSW comment - I decided to use job duties instead of title
...
#Chart Calls by Day Use the code above and add a ggplot chart to it
...
SF %>% 
  ...
  group_by(day) %>% 
  ggplot(data=Days) +
  geom_col(mapping=aes(x=day, y=n, fill=n)) 
...
#Export to spreadsheet - this didn't work
...
#Question: Identify the top 5 days with the most calls? Pro tip: slice_max to the rescue
