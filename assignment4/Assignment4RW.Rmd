...
**#rsw comment: good job with the examples here**
...
read_all_texts <- function(folder_path) {
  ...
  text_data <- readtext(txt_files) %>%
    ...
    mutate(
      ...
      year = ifelse(str_detect(doc_id, "\\d{4}"), as.numeric(str_extract(doc_id, "\\d{4}")), NA) 
    )
  
  return(text_data)
}

text_data <- read_all_texts(folder_path)

message("Sample of the data:")
...
#rsw comment: Good. now write a little narrative about what this means
...
#Load the data
```{r}
#setwd("/Users/teona/Desktop/text analysis/assignment4") #don't write this code in your projects. it causes the chunk to fail on others' machines:setwd("/Users/teona/Desktop/ is your laptop! no one can run this code. i am commenting it out so the code will run
...
```{r}
#rsw comment: I would use cat( to describe what these results mean, for example
...
#rsw comment: this failed for the 23 AI articles that didn't have a year in the title: 	
#moley_perspective_AI_1
#my suggestion: build an index by importing "extracted AI moley index nov 20.csv, join it with the Moley_newsweek articles and then you will have all years


#rsw comment: good graphic, but flawed data
